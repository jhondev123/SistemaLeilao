        ...
        ILogger<CreateAuctionHandler> logger) : IRequestHandler<CreateAuctionCommand, Result<CreateAuctionResponseDto?>>
    {
        public async Task<Result<CreateAuctionResponseDto?>> Handle(CreateAuctionCommand request, CancellationToken ct)
        {
            ...
            if(result > 0)
            {
                ...
                return Result<CreateAuctionResponseDto?>.Success((CreateAuctionResponseDto)auction, new SuccessMessage(nameof(Messages.SuccessAuctionCreated),Messages.SuccessAuctionCreated));
            }
            logger.LogError("Erro ao criar leilão.");
            return Result<CreateAuctionResponseDto?>.Failure(new ErrorMessage(nameof(Messages.ErrorOnCreateAuction), Messages.ErrorOnCreateAuction));
        }
    }
}
